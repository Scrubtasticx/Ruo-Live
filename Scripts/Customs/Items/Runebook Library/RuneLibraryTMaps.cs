...
{
	public class RuneLibraryTMaps : Item
	{
      ...
      {
         CommandSystem.Register( "RuneLibraryTMaps", AccessLevel.Administrator, new CommandEventHandler( RuneLibraryTMaps_OnCommand ) );
      }

		public static void RuneLibraryTMaps_OnCommand( CommandEventArgs args )
		{
			Mobile m = args.Mobile;
         	RuneLibraryTMaps rl = new RuneLibraryTMaps(m);
		}
		...
		[Constructable]
		public RuneLibraryTMaps (Mobile from)
		{
			...
			for( int i=0; i<size; i++ )
			{
				...
				{
					rb.Name = "Treasure Hunting Maps " + nameStart + " - " + (nameEnd-1);
					library.Add(rb);
					rb = new Runebook(0);
					nameStart = nameEnd;
				}
		...
		public RuneLibraryTMaps( Serial serial ) : base( serial )
		{
		}
