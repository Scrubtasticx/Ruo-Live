...
{
	public class MostLosesPvpGump : Gump
	{
		public MostLosesPvpGump() : base( 50, 50 )
		{
			Closable=false;
			Disposable=false;
			Dragable=false;
			...
			string time = "View top 3 players with most overall loses.";
			...
			foreach ( Mobile check2 in mobs )
			{
				if ( check2 is PlayerMobile )
				{
					...
					if ( pvper2.TotalLoses > Mid.TotalLoses && pvper2 != Top && pvper2.AccessLevel == AccessLevel.Player ) 
						Mid = pvper2;
				}
			}
			...
			foreach ( Mobile check3 in mobs )
			{
				if ( check3 is PlayerMobile )
				{
					...
					if ( pvper3.TotalLoses > Low.TotalLoses && pvper3 != Top && pvper3 != Mid && pvper3.AccessLevel == AccessLevel.Player ) 
						Low = pvper3;
				}
			}
