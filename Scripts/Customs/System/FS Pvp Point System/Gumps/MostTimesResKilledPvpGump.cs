...
{
	public class MostTimesResKilledPvpGump : Gump
	{
		public MostTimesResKilledPvpGump() : base( 50, 50 )
		{
			Closable=false;
			Disposable=false;
			Dragable=false;
			...
			string time = "View top 3 players with most overall time res killed.";
			...
			foreach ( Mobile check1 in mobs )
			{
				if ( check1 is PlayerMobile )
				{
					...
					if ( pvper.TotalResKilled > Top.TotalResKilled && pvper.AccessLevel == AccessLevel.Player ) 
						Top = pvper;
				}
			}
			...
			foreach ( Mobile check2 in mobs )
			{
				if ( check2 is PlayerMobile )
				{
					...
					if ( pvper2.TotalResKilled > Mid.TotalResKilled && pvper2 != Top && pvper2.AccessLevel == AccessLevel.Player ) 
						Mid = pvper2;
				}
			}
			...
			foreach ( Mobile check3 in mobs )
			{
				if ( check3 is PlayerMobile )
				{
					...
					if ( pvper3.TotalResKilled > Low.TotalResKilled && pvper3 != Top && pvper3 != Mid && pvper3.AccessLevel == AccessLevel.Player ) 
						Low = pvper3;
				}
			}
