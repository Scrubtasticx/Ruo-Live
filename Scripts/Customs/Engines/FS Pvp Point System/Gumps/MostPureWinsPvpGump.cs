...
{
	public class MostPureWinsPvpGump : Gump
	{
		public MostPureWinsPvpGump() : base( 50, 50 )
		{
			Closable=true;
			Disposable=false;
			Dragable=true;
			...
			string time = "View top 3 players with most overall pure wins.";
			...
			foreach ( Mobile check1 in mobs )
			{
				if ( check1 is PlayerMobile )
				{
					...
					int checkPure = pvper.TotalWins - pvper.TotalLoses - pvper.TotalResKills;
					int checkPureTop = Top.TotalWins - Top.TotalLoses - Top.TotalResKills;

					if ( checkPure > checkPureTop && pvper.AccessLevel == AccessLevel.Player ) 
						Top = pvper;
				}
			}
			...
			foreach ( Mobile check2 in mobs )
			{
				if ( check2 is PlayerMobile )
				{
					...
					int checkPure2 = pvper2.TotalWins - pvper2.TotalLoses - pvper2.TotalResKills;
					int checkPureMid = Mid.TotalWins - Mid.TotalLoses - Mid.TotalResKills;

					if ( checkPure2 > checkPureMid && pvper2 != Top && pvper2.AccessLevel == AccessLevel.Player ) 
						Mid = pvper2;
				}
			}
			...
			foreach ( Mobile check3 in mobs )
			{
				if ( check3 is PlayerMobile )
				{
					...
					int checkPure3 = pvper3.TotalWins - pvper3.TotalLoses - pvper3.TotalResKills;
					int checkPureLow = Low.TotalWins - Low.TotalLoses - Low.TotalResKills;

					if ( checkPure3 > checkPureLow && pvper3 != Top && pvper3 != Mid && pvper3.AccessLevel == AccessLevel.Player ) 
						Low = pvper3;
				}
			}
