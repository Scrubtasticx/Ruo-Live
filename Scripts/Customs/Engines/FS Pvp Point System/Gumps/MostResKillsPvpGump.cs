...
{
	public class MostResKillsPvpGump : Gump
	{
		public MostResKillsPvpGump() : base( 50, 50 )
		{
			Closable=true;
			Disposable=false;
			Dragable=true;
			...
			string time = "View top 3 players with most overall res kills.";
			...
			foreach ( Mobile check1 in mobs )
			{
				if ( check1 is PlayerMobile )
				{
					...
					if ( pvper.TotalResKills > Top.TotalResKills && pvper.AccessLevel == AccessLevel.Player ) 
						Top = pvper;
				}
			}
			...
			foreach ( Mobile check2 in mobs )
			{
				if ( check2 is PlayerMobile )
				{
					...
					if ( pvper2.TotalResKills > Mid.TotalResKills && pvper2 != Top && pvper2.AccessLevel == AccessLevel.Player ) 
						Mid = pvper2;
				}
			}
			...
			foreach ( Mobile check3 in mobs )
			{
				if ( check3 is PlayerMobile )
				{
					...
					if ( pvper3.TotalResKills > Low.TotalResKills && pvper3 != Top && pvper3 != Mid && pvper3.AccessLevel == AccessLevel.Player ) 
						Low = pvper3;
				}
			}
